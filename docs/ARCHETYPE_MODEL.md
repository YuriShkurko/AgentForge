...
Provider Runtime v0 is available only inside this model-driven path. It is intentionally bounded: generated apps may configure read-only `github_issues` or `http_json` providers that target an existing `model.imports` config. Providers fetch and normalize records, then reuse the generic importer pipeline for mapping, validation, upsert/idempotency, import-run history, and frontend preview/sync. The HTTP JSON provider reads a configured URL, can send an optional bearer token, and extracts records from a top-level array, `records`/`items`/`data` wrapper, or a simple dotted `records_path`. Provider Runtime v0 does not add OAuth, write-back, arbitrary connectors, provider marketplaces, scheduled sync, or live-network requirements for default validation.

Example generated apps: Client Onboarding Workspace, Vendor Risk Tracker, GitHub Issues Workspace, and HTTP JSON Vendor Feed. They share the same model-driven generator path but produce different entities, routes, labels, enum values, pages, workflow actions, seed data, dashboard cards, accent colors, entity layouts, page compositions, visual recipes, and optional read-only provider configs. Client Onboarding uses a board workspace composition with the `workspace_board` recipe; Vendor Risk uses a register table composition with the `executive_register` recipe.
