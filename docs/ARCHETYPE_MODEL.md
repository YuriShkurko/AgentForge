...
Provider Runtime v0 is available only inside this model-driven path. It is intentionally bounded: generated apps may configure read-only GitHub Issues providers that target an existing `model.imports` config. The provider fetches and normalizes issue records, then reuses the generic importer pipeline for mapping, validation, upsert/idempotency, import-run history, and frontend preview/sync. It does not add OAuth, write-back, arbitrary connectors, provider marketplaces, or live-network requirements for default validation.

Example generated apps: Client Onboarding Workspace, Vendor Risk Tracker, and GitHub Issues Workspace. They share the same model-driven generator path but produce different entities, routes, labels, enum values, pages, workflow actions, seed data, dashboard cards, accent colors, entity layouts, page compositions, and visual recipes. Client Onboarding uses a board workspace composition with the `workspace_board` recipe; Vendor Risk uses a register table composition with the `executive_register` recipe.
