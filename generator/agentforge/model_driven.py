...
const heroHeadline = (): string => {{ const recipe = recipeId(); if (useClientWorkspace()) return clientWorkHeroHeadline(); if (model.ui.dashboard.headline) return model.ui.dashboard.headline; if (usePipelineBoard()) return 'Move work through stages'; if (recipe === 'approval_review_queue') return 'Review the queue and record decisions'; if (useInventoryOps()) return 'Track assets, stock, and upkeep'; return appName(); }};
const heroSummary = (): string => {{ const recipe = recipeId(); if (useClientWorkspace()) return 'Open a client, review related work or sessions, and keep payment status visible without leaving the workspace.'; if (model.ui.dashboard.summary) return model.ui.dashboard.summary; if (usePipelineBoard()) return 'Track active cards across stages, spot next follow-ups, and keep applications or deals moving.'; if (recipe === 'approval_review_queue') return 'Triage items needing review, use claim/approve/reject actions, and keep decision history visible.'; if (useInventoryOps()) return 'Monitor assets or inventory by status, quantity, location, vendor, and maintenance or reorder needs.'; return model.app.description || ''; }};
