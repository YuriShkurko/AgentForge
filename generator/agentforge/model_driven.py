...
const emptyForLane = (entity: Entity): string => {{ const singular = (entity.labelSingular || 'record').toLowerCase(); if (recipeId() === 'pipeline_kanban') return `No ${{singular}}s in this stage yet — move work through stages by creating or editing a ${{singular}}.`; if (recipeId() === 'approval_review_queue') return `No ${{singular}}s in this queue state yet.`; return `No ${{singular}}s in this lane yet.`; }};
