...
function StandardLayout(ctx: LayoutContext) {{ const rows = ctx.rowsByEntity[ctx.activeEntity.name] || []; const actions = model.actions.filter((action) => action.entity === ctx.activeEntity.name); return <section className="content"><HeroBanner ctx={{ctx}} entity={{ctx.activeEntity}} count={{rows.length}} /><Dashboard rowsByEntity={{ctx.rowsByEntity}} /><CreateForm entity={{ctx.activeEntity}} form={{ctx.form}} setForm={{ctx.setForm}} save={{ctx.save}} rowsByEntity={{ctx.rowsByEntity}} /><EntityRows entity={{ctx.activeEntity}} rows={{rows}} rowsByEntity={{ctx.rowsByEntity}} actions={{actions}} onAction={{ctx.runAction}} parent={{ctx.primary}} /></section>; }}
