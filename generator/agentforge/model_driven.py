...
const heroHeadline = (): string => model.ui.dashboard.headline || appName();
const heroSummary = (): string => model.ui.dashboard.summary || model.app.description || '';
const emptyForList = (entity: Entity): string => {{ const singular = (entity.labelSingular || 'record').toLowerCase(); const plural = (entity.labelPlural || `${{singular}}s`).toLowerCase(); return `No ${{plural}} yet — load seed data or create your first ${{singular}}.`; }};
...
const emptyForLane = (entity: Entity): string => {{ const singular = (entity.labelSingular || 'record').toLowerCase(); return `No ${{singular}}s in this lane yet.`; }};
