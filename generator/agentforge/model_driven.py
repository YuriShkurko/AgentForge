...
const heroHeadline = (): string => {{ const recipe = recipeId(); if (useClientWorkspace()) return clientWorkHeroHeadline(); if (model.ui.dashboard.headline) return model.ui.dashboard.headline; if (usePipelineBoard()) return 'Move work through stages'; if (recipe === 'approval_review_queue') return 'Review the queue and record decisions'; if (recipe === 'inventory_asset_tracker') return 'Track assets, stock, and upkeep'; return appName(); }};
...
const emptyForLane = (entity: Entity): string => {{ const singular = (entity.labelSingular || 'record').toLowerCase(); if (usePipelineBoard()) return `No ${{singular}}s in this stage yet — move work through stages by creating or editing a ${{singular}}.`; if (recipeId() === 'approval_review_queue') return `No ${{singular}}s in this queue state yet.`; if (recipeId() === 'inventory_asset_tracker') return `No ${{singular}}s in this asset status yet — add stock, equipment, or maintenance records to see the workspace fill in.`; return `No ${{singular}}s in this lane yet.`; }};
