...
def _app_metadata(pack: DomainPack) -> dict[str, str]:
    """Return generated-app-facing app copy, with experience-specific cleanup."""
    display = pack.display_name
    description = pack.domain.product_purpose
    experience = pack.future_extensions.get("experience", {}) if isinstance(pack.future_extensions, dict) else {}
    primitive_id = str(experience.get("primitive_id") or experience.get("experience_id") or "")
    if primitive_id == "pipeline_board":
        haystack = f"{pack.display_name} {description}".lower()
        if any(token in haystack for token in ("hiring", "candidate", "applicant", "application", "job")):
            display = "Hiring Pipeline"
            description = "Move candidates through stages, see what needs attention next, and keep applications, owners, and follow-ups visible."
        elif any(token in haystack for token in ("deal", "lead", "opportunity", "sales", "customer")):
            display = "Sales Pipeline"
            description = "Move deals through stages, spot next follow-ups, and keep customers, owners, and value context visible."
        else:
            display = "Pipeline Board"
            description = "Move work through stages, spot next follow-ups, and keep owners and supporting context visible."
    return {"name": pack.name, "displayName": display, "description": description}
...
const primaryEntity = (): Entity => findEntity(model.ui.focus.primary_entity || model.ui.dashboard.primary_entity);
const primaryDisplayLayout = (): Display['layout'] => display(primaryEntity()).layout;
const isRelationBoardLayout = (): boolean => model.ui.composition === 'board_workspace' && primaryDisplayLayout() === 'board_by_relation';
const isPipelineBoardExperience = (): boolean => experienceId() === 'pipeline_board' || primitiveId() === 'pipeline_board';
const usePipelineBoard = (): boolean => isPipelineBoardExperience() || (!hasExperienceMetadata() && (recipeId() === 'pipeline_kanban' || isRelationBoardLayout()));
const useBoardWorkspace = (): boolean => usePipelineBoard() || model.ui.composition === 'board_workspace';
const isInventoryOpsExperience = (): boolean => experienceId() === 'inventory_ops' || primitiveId() === 'inventory_ops';
const hasInventorySurfaceMetadata = (): boolean => Boolean(model.entities.find((entity) => ['asset','inventory_item'].includes(entity.name)) && model.entities.some((entity) => ['category','location','vendor','supplier','maintenance_task','warehouse'].includes(entity.name)));
const useInventoryOps = (): boolean => isInventoryOpsExperience() || (!hasExperienceMetadata() && (recipeId() === 'inventory_asset_tracker' || hasInventorySurfaceMetadata()));
const clientWorkHeroHeadline = (): string => {{ const text = `${{model.app.displayName}} ${{model.app.description}} ${{model.entities.map((entity) => entity.name).join(' ')}}`.toLowerCase(); if (/freelance|designer|project|invoice|work history/.test(text)) return 'Manage clients, work, and payments'; if (/tutor|student|lesson|homework|math/.test(text)) return 'Manage students, sessions, and payments'; if (/coach|goal|training/.test(text) && !/pipeline|application|candidate|hiring/.test(text)) return 'Manage clients, sessions, and goals'; return 'Manage clients, work, and payments'; }};
const pipelineVocabulary = () => {{ const text = `${{model.app.displayName}} ${{model.app.description}} ${{model.entities.map((entity) => `${{entity.name}} ${{entity.labelPlural}}`).join(' ')}}`.toLowerCase(); const isHiring = /hiring|candidate|applicant|application|job/.test(text); const isSales = /deal|lead|opportunity|sales|customer/.test(text); if (isHiring) return {{ title: 'Hiring Pipeline', summary: 'Move candidates through stages, see what needs attention next, and keep applications, owners, and follow-ups visible.', workPlural: 'Applications', contextPlural: 'Companies / owners', laneNoun: 'stage', boardHeading: 'Applications by stage', boardSummary: 'Review active applications by stage and spot the next follow-up.', supportingCopy: 'Companies, owners, and follow-ups stay connected to each application.' }}; if (isSales) return {{ title: 'Sales Pipeline', summary: 'Move deals through stages, spot next follow-ups, and keep customers, owners, and value context visible.', workPlural: 'Deals', contextPlural: 'Customers / owners', laneNoun: 'stage', boardHeading: 'Deals by stage', boardSummary: 'Review active deals by stage and keep the next follow-up visible.', supportingCopy: 'Customers, owners, and follow-ups stay connected to each deal.' }}; return {{ title: 'Pipeline Board', summary: 'Move work through stages, spot next follow-ups, and keep owners and supporting context visible.', workPlural: 'Cards', contextPlural: 'Supporting context', laneNoun: 'stage', boardHeading: 'Work by stage', boardSummary: 'Review active work by stage and keep attention items visible.', supportingCopy: 'Owners, notes, and follow-ups stay connected to each card.' }}; }};
const heroHeadline = (): string => {{ const recipe = recipeId(); if (useClientWorkspace()) return clientWorkHeroHeadline(); if (usePipelineBoard()) return pipelineVocabulary().title; if (model.ui.dashboard.headline) return model.ui.dashboard.headline; if (recipe === 'approval_review_queue') return 'Review the queue and record decisions'; if (useInventoryOps()) return 'Track assets, stock, and upkeep'; return appName(); }};
const heroSummary = (): string => {{ const recipe = recipeId(); if (useClientWorkspace()) return 'Open a client, review related work or sessions, and keep payment status visible without leaving the workspace.'; if (usePipelineBoard()) return pipelineVocabulary().summary; if (model.ui.dashboard.summary) return model.ui.dashboard.summary; if (recipe === 'approval_review_queue') return 'Triage items needing review, use claim/approve/reject actions, and keep decision history visible.'; if (useInventoryOps()) return 'Monitor assets or inventory by status, quantity, location, vendor, and maintenance or reorder needs.'; return model.app.description || ''; }};
const emptyForList = (entity: Entity): string => {{ const singular = (entity.labelSingular || 'record').toLowerCase(); const plural = (entity.labelPlural || `${{singular}}s`).toLowerCase(); const recipe = recipeId(); if (useClientWorkspace() && ['client','session','payment'].includes(entity.name)) return `No ${{plural}} yet — load seed data or create the first ${{singular}} to start the session workflow.`; if (recipe === 'approval_review_queue' && ['item','decision','reviewer'].includes(entity.name)) return `No ${{plural}} yet — load seed data or create a review item to start the queue.`; if (useInventoryOps() && ['asset','inventory_item','category','location','vendor','supplier','maintenance_task','warehouse'].includes(entity.name)) return `No ${{plural}} yet — load seed data or create the first ${{singular}} to start tracking assets, stock, and maintenance.`; return `No ${{plural}} yet — load seed data or create your first ${{singular}}.`; }};
...
const emptyForLane = (entity: Entity): string => {{ const singular = (entity.labelSingular || 'record').toLowerCase(); if (usePipelineBoard()) return `No ${{singular}}s in this stage yet — move work through stages by creating or editing a ${{singular}}.`; if (recipeId() === 'approval_review_queue') return `No ${{singular}}s in this queue state yet.`; if (useInventoryOps()) return `No ${{singular}}s in this asset status yet — add stock, equipment, or maintenance records to see the workspace fill in.`; return `No ${{singular}}s in this lane yet.`; }};
...
export default function App() {{
  ...
  return <main className={{shellClass}} data-composition={{model.ui.composition}} data-recipe={{model.ui.recipe}} data-active-entity={{active}} data-primary-active={{isPrimaryActive ? 'true' : 'false'}} data-imports-active={{importsMode ? 'true' : 'false'}} data-providers-active={{providersMode ? 'true' : 'false'}}>
    ...
    {{providersMode ? <ProviderPanel reload={{loadAll}} /> : importsMode ? <ImportPanel reload={{loadAll}} /> : model.ui.composition === 'standard' ? <StandardLayout {{...context}} /> : isPrimaryActive && useClientWorkspace() ? <ClientWorkWorkspace {{...context}} /> : isPrimaryActive && usePipelineBoard() ? <PipelineBoardWorkspace {{...context}} /> : isPrimaryActive && useBoardWorkspace() ? <BoardWorkspace {{...context}} /> : isPrimaryActive && model.ui.composition === 'register_table' ? <RegisterTable {{...context}} /> : <FocusedSurface {{...context}} />}}
  </main>;
}}

function Sidebar({{ active, setActive, seed }}: {{ active: string; setActive: (name: string) => void; seed: () => void }}) {{ return <aside><p className="eyebrow">AgentForge model-driven app</p><h1>{{appName()}}</h1><p>{{model.app.description}}</p><button onClick={{seed}}>Load seed data</button>{{model.entities.map((item) => <button className={{item.name === active ? 'active' : ''}} key={{item.name}} onClick={{() => setActive(item.name)}}>{{item.labelPlural}}</button>)}}{{model.imports.length > 0 && <button className={{active === '__imports__' ? 'active' : ''}} data-ui-control="imports-nav" onClick={{() => setActive('__imports__')}}>Imports</button>}}{{model.providers.length > 0 && <button className={{active === '__providers__' ? 'active' : ''}} data-ui-control="providers-nav" onClick={{() => setActive('__providers__')}}>Providers</button>}}</aside>; }}
...
type ClientWorkVocabulary = {{ clientSingular: string; workSingular: string; workPlural: string; paymentSingular: string; paymentPlural: string; detailCopy: string; timelineHeading: string }};
function clientWorkVocabulary(): ClientWorkVocabulary {{
  const text = `${{model.app.displayName}} ${{model.app.description}} ${{model.entities.map((entity) => `${{entity.name}} ${{entity.labelPlural}}`).join(' ')}}`.toLowerCase();
  const isTutor = /tutor|student|lesson|homework|math/.test(text);
  const isFreelance = /freelance|designer|project|invoice|brand|landing page|work history/.test(text);
  const isCoach = /coach|goal|training/.test(text) && !isTutor;
  const paymentEntity = model.entities.find((entity) => ['invoice','payment','earning'].includes(entity.name));
  const paymentSingular = paymentEntity?.labelSingular.toLowerCase() || 'payment';
  const paymentPlural = paymentEntity?.labelPlural.toLowerCase() || 'payments';
  if (isFreelance) return {{ clientSingular: 'client', workSingular: 'work item', workPlural: 'work history', paymentSingular, paymentPlural, detailCopy: 'Inspect the client, completed work, open projects, and invoice/payment status in one place.', timelineHeading: 'Work and payment timeline' }};
  if (isTutor) return {{ clientSingular: 'student', workSingular: 'session', workPlural: 'sessions', paymentSingular, paymentPlural, detailCopy: 'Review each student, upcoming/completed sessions, and logged payments together.', timelineHeading: 'Session and payment timeline' }};
  if (isCoach) return {{ clientSingular: 'client', workSingular: 'session', workPlural: 'sessions / goals', paymentSingular, paymentPlural, detailCopy: 'Review each client, session progress, goals, and payments together.', timelineHeading: 'Session, goal, and payment timeline' }};
  return {{ clientSingular: 'client', workSingular: 'work item', workPlural: 'work / sessions', paymentSingular, paymentPlural, detailCopy: 'Review the client, related work, sessions, and payments together.', timelineHeading: 'Client activity timeline' }};
}}
function relationFieldNamesFor(entityName: string): string[] {{ return [`${{entityName}}_id`, entityName]; }}
function rowRelatesTo(row: Row, entityName: string, id: number): boolean {{ if (!Number.isFinite(id) || id <= 0) return false; return relationFieldNamesFor(entityName).some((field) => Number(row[field]) === id); }}
function recordTitle(entity: Entity, row: Row, rowsByEntity: RowMap): string {{ const fieldName = display(entity).title_field || inferTitleField(entity); const field = fieldFor(entity, fieldName); return cellValue(field, row[fieldName], rowsByEntity) || `${{entity.labelSingular}} #${{row.id}}`; }}
function recordSubtitle(entity: Entity, row: Row, rowsByEntity: RowMap): string {{ const fields = entity.fields.filter((field) => field.name !== (display(entity).title_field || inferTitleField(entity))); const picked = fields.find((field) => field.type === 'enum') || fields.find((field) => field.type === 'date') || fields.find((field) => field.type === 'relation') || fields[0]; return picked ? `${{picked.label}}: ${{cellValue(picked, row[picked.name], rowsByEntity)}}` : ''; }}

function ClientWorkWorkspace(ctx: LayoutContext) {{
  const vocabulary = clientWorkVocabulary();
  const clientEntity = model.entities.find((item) => ['client','student','customer'].includes(item.name)) || ctx.secondary || model.entities[0];
  const workEntity = ctx.primary;
  const paymentEntity = model.entities.find((item) => ['payment','invoice','earning'].includes(item.name));
  const clientRows = uniqueById(asRows(ctx.rowsByEntity[clientEntity.name]));
  const workRows = asRows(ctx.rowsByEntity[workEntity.name]);
  const paymentRows = paymentEntity ? asRows(ctx.rowsByEntity[paymentEntity.name]) : [];
  const selectedClient = clientRows[0];
  const selectedClientId = typeof selectedClient?.id === 'number' ? selectedClient.id : Number(selectedClient?.id || 0);
  const relatedWork = selectedClientId ? workRows.filter((row) => rowRelatesTo(row, clientEntity.name, selectedClientId)) : workRows.slice(0, 3);
  const relatedPayments = selectedClientId ? paymentRows.filter((row) => rowRelatesTo(row, clientEntity.name, selectedClientId) || relatedWork.some((work) => rowRelatesTo(row, workEntity.name, Number(work.id || 0)))) : paymentRows.slice(0, 3);
  const actions = model.actions.filter((action) => action.entity === workEntity.name);
  return <section className="content client-workspace" data-ui-layout="client-workspace" data-ui-surface="client-workspace"><HeroBanner ctx={{ctx}} entity={{workEntity}} count={{workRows.length}} headingSuffix={{workEntity.labelPlural}} /><Dashboard rowsByEntity={{ctx.rowsByEntity}} compact /><div className="client-workspace-main"><section className="client-focus card" data-ui-surface="client-detail"><p className="eyebrow">Selected {{vocabulary.clientSingular}}</p><h3>{{selectedClient ? recordTitle(clientEntity, selectedClient, ctx.rowsByEntity) : `Load seed data to inspect a ${{vocabulary.clientSingular}}`}}</h3><p>{{vocabulary.detailCopy}}</p>{{selectedClient && <div className="client-metrics"><span><b>{{relatedWork.length}}</b> {{vocabulary.workPlural}}</span><span><b>{{relatedPayments.length}}</b> {{vocabulary.paymentPlural}}</span></div>}}<div className="client-list">{{clientRows.length === 0 ? <EmptyState text={{emptyForList(clientEntity)}} compact /> : clientRows.slice(0, 4).map((row) => <article key={{row.id}} className="mini-client"><b>{{recordTitle(clientEntity, row, ctx.rowsByEntity)}}</b><small>{{row.email || row.contact || 'Client record ready'}}</small></article>)}}</div></section><section className="activity-panel card" data-ui-surface="client-activity-timeline"><p className="eyebrow">Recent activity</p><h3>{{vocabulary.timelineHeading}}</h3><div className="timeline-list">{{relatedWork.length === 0 && relatedPayments.length === 0 ? <EmptyState text={{`No linked ${{vocabulary.workPlural}} or ${{vocabulary.paymentPlural}} yet — load seed data to see the client story.`}} /> : [...relatedWork.slice(0, 4).map((row) => ({{ kind: vocabulary.workSingular, row, entity: workEntity }})), ...relatedPayments.slice(0, 3).map((row) => ({{ kind: vocabulary.paymentSingular, row, entity: paymentEntity || workEntity }}))].map((item, index) => <article className="timeline-item" key={{`${{item.kind}}-${{item.row.id}}-${{index}}`}}><span>{{item.kind}}</span><b>{{recordTitle(item.entity, item.row, ctx.rowsByEntity)}}</b><small>{{recordSubtitle(item.entity, item.row, ctx.rowsByEntity)}}</small></article>)}}</div></section><aside className="client-work-create"><CreateForm entity={{ctx.activeEntity}} form={{ctx.form}} setForm={{ctx.setForm}} save={{ctx.save}} rowsByEntity={{ctx.rowsByEntity}} compact /></aside></div><EntityRows entity={{workEntity}} rows={{workRows}} rowsByEntity={{ctx.rowsByEntity}} actions={{actions}} onAction={{ctx.runAction}} forcedLayout="cards" parent={{clientEntity}} /></section>;
}}

function PipelineBoardWorkspace(ctx: LayoutContext) {{
  const vocabulary = pipelineVocabulary();
  const primaryRows = asRows(ctx.rowsByEntity[ctx.primary.name]);
  const secondaryEntities = model.entities.filter((entity) => entity.name !== ctx.primary.name);
  const actions = model.actions.filter((action) => action.entity === ctx.primary.name);
  const primaryLayout: Display['layout'] = display(ctx.primary).layout === 'board_by_relation' ? 'board_by_relation' : 'board_by_status';
  return <section className="content pipeline-workspace" data-ui-layout="pipeline-board-workspace" data-ui-surface="pipeline-board-workspace"><HeroBanner ctx={{ctx}} entity={{ctx.primary}} count={{primaryRows.length}} headingSuffix={{vocabulary.workPlural}} /><section className="pipeline-board-focus" data-ui-surface="pipeline-board"><div className="pipeline-board-heading"><div><p className="eyebrow">{{vocabulary.workPlural}}</p><h3>{{vocabulary.boardHeading}}</h3><p>{{vocabulary.boardSummary}}</p></div><button type="button" className="hero-secondary" onClick={{ctx.seed}}>Load seed data</button></div><EntityRows entity={{ctx.primary}} rows={{primaryRows}} rowsByEntity={{ctx.rowsByEntity}} actions={{actions}} onAction={{ctx.runAction}} forcedLayout={{primaryLayout}} groupBy={{model.ui.focus.group_by}} parent={{undefined}} /></section><section className="pipeline-supporting" data-ui-surface="pipeline-supporting"><div className="pipeline-supporting-copy card"><p className="eyebrow">Supporting context</p><h3>{{vocabulary.contextPlural}}</h3><p>{{vocabulary.supportingCopy}}</p></div>{{secondaryEntities.map((entity) => <article className="pipeline-support-card card" key={{entity.name}}><h3>{{section_heading_secondary(entity)}}</h3><EntityRows entity={{entity}} rows={{uniqueById(asRows(ctx.rowsByEntity[entity.name])).slice(0, 4)}} rowsByEntity={{ctx.rowsByEntity}} actions={{[]}} onAction={{ctx.runAction}} forcedLayout="cards" parent={{ctx.primary}} /></article>)}}<section className="compact-create pipeline-create"><CreateForm entity={{ctx.activeEntity}} form={{ctx.form}} setForm={{ctx.setForm}} save={{ctx.save}} rowsByEntity={{ctx.rowsByEntity}} compact /></section></section></section>;
}}
...
function recipeHighlightCards(rowsByEntity: RowMap): JSX.Element[] {{
  ...
  if (usePipelineBoard()) {{
    ...
    const relationField = fieldFor(primary, model.ui.focus.group_by) || primary.fields.find((field) => field.type === 'relation');
    const laneEntity = relationField?.targetEntity ? entityByName(relationField.targetEntity) : undefined;
    const cards = asRows(rowsByEntity[primary.name]);
    const stages = laneEntity ? asRows(rowsByEntity[laneEntity.name]) : [];
    const withDue = cards.filter((row) => String(row.due_on ?? '').trim()).length;
    const vocabulary = pipelineVocabulary();
    return [<article className="stat-card recipe-highlight" data-ui-surface="recipe-highlight" key="pipeline-stages"><p>Pipeline stages</p><strong>{{stages.length}}</strong><small>Move work through stages</small></article>, <article className="stat-card recipe-highlight" data-ui-surface="recipe-highlight" key="pipeline-active"><p>{{vocabulary.workPlural}}</p><strong>{{cards.length}}</strong><small>Applications, deals, jobs, or tickets</small></article>, <article className="stat-card recipe-highlight" data-ui-surface="recipe-highlight" key="pipeline-followups"><p>Next follow-ups</p><strong>{{withDue}}</strong><small>Cards with due dates</small></article>];
  }}
  if (useClientWorkspace()) {{
    ...
    const paymentEntity = model.entities.find((entity) => ['payment','invoice','earning'].includes(entity.name));
    const workRows = asRows(rowsByEntity[workEntity.name]);
    const clients = clientEntity ? asRows(rowsByEntity[clientEntity.name]) : [];
    const payments = paymentEntity ? asRows(rowsByEntity[paymentEntity.name]) : [];
    const completed = workRows.filter((row) => ['completed','delivered','paid'].includes(String(row.status ?? ''))).length;
    const unpaid = payments.filter((row) => ['sent','draft','expected','due','pending'].includes(String(row.status ?? ''))).length;
    return [<article className="stat-card recipe-highlight" data-ui-surface="recipe-highlight" key="client-clients"><p>{{titleize(vocabulary.clientSingular)}}s / students</p><strong>{{clients.length}}</strong><small>People and organizations you work with</small></article>, <article className="stat-card recipe-highlight" data-ui-surface="recipe-highlight" key="client-work"><p>{{vocabulary.workPlural === 'work history' ? 'Recent work' : 'Upcoming / recent sessions'}}</p><strong>{{Math.max(workRows.length - completed, 0)}}</strong><small>{{vocabulary.workPlural}}</small></article>, <article className="stat-card recipe-highlight" data-ui-surface="recipe-highlight" key="client-payments"><p>{{unpaid > 0 ? 'Unpaid / pending payments' : 'Payments logged'}}</p><strong>{{unpaid > 0 ? unpaid : payments.length}}</strong><small>{{vocabulary.paymentPlural}}</small></article>];
  }}
...
function EntityRows({{ entity, rows, rowsByEntity, actions, onAction, forcedLayout, groupBy, register = false, parent }}: {{ entity: Entity; rows: Row[]; rowsByEntity: RowMap; actions: Action[]; onAction: (target: Entity, name: string, id: number) => void; forcedLayout?: Display['layout']; groupBy?: string; register?: boolean; parent?: Entity }}) {{ const view = display(entity); const layout = forcedLayout || view.layout; const sectionTitle = titleize(entity.labelPlural); if (layout === 'cards') return <div className="entity-grid" data-ui-layout="cards">{{rows.length === 0 ? <EmptyState text={{emptyForRelated(entity, parent)}} /> : rows.map((row) => <RecordCard key={{row.id}} entity={{entity}} row={{row}} rowsByEntity={{rowsByEntity}} actions={{actions}} onAction={{onAction}} />)}}</div>; if (layout === 'board_by_status') {{ const status = fieldFor(entity, groupBy || view.badge_field) || entity.fields.find((item) => item.type === 'enum'); return <div className="board-scroll" data-ui-layout="board_by_status"><div className="board">{{(status?.enumValues || ['records']).map((lane) => {{ const laneRows = rows.filter((row) => !status || String(row[status.name]) === lane); return <section className="lane" key={{lane}}><div className="lane-heading"><h3>{{humanize(lane)}}</h3><small>{{laneRows.length}} {{laneRows.length === 1 ? entity.labelSingular.toLowerCase() : entity.labelPlural.toLowerCase()}}</small></div>{{laneRows.length === 0 ? <EmptyState text={{emptyForLane(entity)}} compact /> : laneRows.map((row) => usePipelineBoard() ? <PipelineRecordCard key={{row.id}} entity={{entity}} row={{row}} rowsByEntity={{rowsByEntity}} actions={{actions}} onAction={{onAction}} /> : <RecordCard key={{row.id}} entity={{entity}} row={{row}} rowsByEntity={{rowsByEntity}} actions={{actions}} onAction={{onAction}} />)}}</section>; }})}}</div></div>; }}
  if (layout === 'board_by_relation') {{
    ...
    return <div className="board-scroll" data-ui-layout="board_by_relation"><div className="board">{{laneRows.map((lane) => {{
      ...
      return <section className="lane" key={{laneId}} data-lane-id={{laneId}}><div className="lane-heading"><h3>{{laneTitle}}</h3><small>{{matched.length}} {{matched.length === 1 ? entity.labelSingular.toLowerCase() : entity.labelPlural.toLowerCase()}}</small></div>{{matched.length === 0 ? <EmptyState text={{emptyForLane(entity)}} compact /> : matched.map((row) => usePipelineBoard() ? <PipelineRecordCard key={{row.id}} entity={{entity}} row={{row}} rowsByEntity={{rowsByEntity}} actions={{actions}} onAction={{onAction}} /> : <RecordCard key={{row.id}} entity={{entity}} row={{row}} rowsByEntity={{rowsByEntity}} actions={{actions}} onAction={{onAction}} />)}}</section>;
    }})}}</div></div>;
...
function PipelineRecordCard({{ entity, row, rowsByEntity, actions, onAction }}: {{ entity: Entity; row: Row; rowsByEntity: RowMap; actions: Action[]; onAction: (target: Entity, name: string, id: number) => void }}) {{
  const title = recordTitle(entity, row, rowsByEntity);
  const laneFieldName = model.ui.focus.group_by || '';
  const contextFields = entity.fields.filter((field) => field.type === 'relation' && field.name !== laneFieldName);
  const dueField = entity.fields.find((field) => ['due_on','due_date','deadline','next_follow_up','follow_up_on'].includes(field.name) || field.semantic === 'due_date');
  const valueField = entity.fields.find((field) => ['value','amount','budget','salary','fee'].includes(field.name));
  const notesField = entity.fields.find((field) => ['notes','note','description'].includes(field.name) || field.semantic === 'description');
  const contextItems = contextFields.map((field) => ({{ label: field.label || titleize(field.name), value: cellValue(field, row[field.name], rowsByEntity) }})).filter((item) => item.value);
  const due = dueField ? cellValue(dueField, row[dueField.name], rowsByEntity) : '';
  const amount = valueField ? cellValue(valueField, row[valueField.name], rowsByEntity) : '';
  const notes = notesField ? cellValue(notesField, row[notesField.name], rowsByEntity) : '';
  return <article className="record-card pipeline-card" data-ui-surface="pipeline-card"><div><p className="eyebrow">Pipeline card</p><h3>{{title || `${{entity.labelSingular}} #${{row.id}}`}}</h3>{{contextItems.length > 0 && <div className="pipeline-card-context">{{contextItems.map((item) => <p key={{item.label}}>{{item.label}}: {{item.value}}</p>)}}</div>}}<div className="pipeline-card-meta">{{due && <small>Follow-up: {{due}}</small>}}{{amount && <small>Value: {{amount}}</small>}}</div>{{notes && <small>{{notes}}</small>}}</div><div>{{actions.map((action) => <button key={{action.name}} onClick={{() => row.id && onAction(entity, action.name, row.id)}}>{{action.label || humanize(action.name)}}</button>)}}</div></article>;
}}
